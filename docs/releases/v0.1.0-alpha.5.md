...
- fix for supported screenshots that OCR into narrower traditional or truncated
  labels such as `对英雄输出`, truncated `对英雄出`, or abbreviated `团率`
- diagnostics export now includes OCR text for failed import or recognition
  attempts
...
If you already installed `v0.1.0-alpha.4`, install this newer APK over it.
...
export into your support message. The export still excludes the original
screenshot and full saved record payload, but it may now include OCR text from
the failed recognition attempt so support can inspect what ML Kit actually
read.
